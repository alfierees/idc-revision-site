...
$$\begin{gathered}
Q_1 = 5-2 = 3,\quad Q_2 = 15-2 = 13, \\
Q_3 = 20\ (\text{perfectly inelastic}).
\end{gathered}$$

$$\begin{aligned}
E[\text{spend}] &= \tfrac12(2\cdot 3) + \tfrac13(2\cdot 13) + \tfrac16(2\cdot 20) \\
&= \tfrac12(6) + \tfrac13(26) + \tfrac16(40)
\end{aligned}$$
...
$$\begin{aligned}
\pi^{\text{fair}} &= \tfrac12(2\cdot 5) + \tfrac13(2\cdot 15) + \tfrac16(2\cdot 20) \\
&= 5 + 10 + 6.67 = 21.67.
\end{aligned}$$
...
$$\begin{aligned}
E[\text{DWL}] &= \tfrac12\underbrace{(\tfrac12\cdot 2\cdot 2)}_{\text{mild}} + \tfrac13\underbrace{(\tfrac12\cdot 2\cdot 2)}_{\text{moderate}} + \tfrac16\cdot 0 \\
&= 1 + 0.67 = 1.67.
\end{aligned}$$
...
$$\begin{aligned}
MR_{\text{pub}} = MC:\quad 100 - 2X_{\text{pub}} &= X_{\text{pub}} + 10 \\
&\Rightarrow 90 = 3X_{\text{pub}} \Rightarrow X_{\text{pub}} = 30.
\end{aligned}$$
...
$$\begin{aligned}
\frac{\partial \pi_1}{\partial p_1} &= (29 - p_1 + \tfrac14 p_2) - (p_1 - 6) = 35 - 2p_1 + \tfrac14 p_2 = 0 \\
&\Rightarrow p_1 = 17.5 + 0.125\,p_2.
\end{aligned}$$
...
$$\begin{gathered}
q = 27 - 2(13) + 13 = 14, \qquad \pi^{\text{gross}} = (13-6)\cdot 14 = 98, \\
\pi^{\text{net}} = 98 - 30 = 68.
\end{gathered}$$
...
$$\begin{gathered}
\underbrace{98 - K}_{\text{adopt}} \;\ge\; \underbrace{196}_{\text{deviate to Don't}} \\
\Longrightarrow K \le -98.
\end{gathered}$$
...
**Inverse demand first.** Part (a)'s market demand is $Q = 440 - 20p$; solve for price to get what the monopolist can charge for a quantity $Q$:
$$p = \tfrac{440 - Q}{20} = 22 - \tfrac{Q}{20}.$$
(Sense-check: choke price $22$ at $Q=0$; slope $-\tfrac1{20}$ — every extra 20 coffees drops the price \$1, matching the $20p$ in $Q = 440 - 20p$.)

The merged firm's $MC = 0.5Q$. The optimal two-part tariff sets the per-unit price at $MC$, so the efficient quantity is where **demand meets $MC$**:
...
$$\begin{aligned}
22 - \tfrac{Q}{5} = 0.5Q &\Rightarrow 22 = 0.7Q \Rightarrow Q = \tfrac{220}{7} \approx 31.4, \\
p &= 22 - \tfrac{Q}{10} = \tfrac{132}{7} \approx \boxed{18.86}.
\end{aligned}$$
...
![[PP01_open2c_mba_optimum.png|660]]

> [!tip] Why $Q=\tfrac{220}{7}$, $p=\tfrac{132}{7}$ — and why $p=11$ fails
> **Left:** with only MBAs, set quantity where $MR=MC$: $22-\tfrac{Q}{5}=0.5Q\Rightarrow 0.7Q=22\Rightarrow Q=\tfrac{220}{7}\approx31.4$. Then read the price *up on the demand curve*: $p=22-\tfrac{Q}{10}=\tfrac{132}{7}\approx18.86$. (Never read the price off the $MR$ curve.)
> **Right:** to pull the price down to $11$ and start attracting students you must sell $Q=110$, but there rising $MC=0.5\cdot110=55$ dwarfs the $\$11$ price — roughly a $\$44$ loss on every marginal cup. So the firm stays put at $18.86$, MBAs only.
