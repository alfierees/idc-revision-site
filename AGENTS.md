...
- `npm run dev` — Astro dev server, default port **4321**. `.Codex/launch.json` pins it to **4322** (`npm run dev -- --port 4322`).
...
Ingestion is driven by the user-level **`/ingest <subject-slug>`** slash command (`~/.Codex/commands/ingest.md`) — it holds the full workflow and conventions; don't duplicate or improvise around it. Its machinery lives in `scripts/ingest/`:
